 ...
 * 
 * <p>Copyright: Copyright (c) 2005-2016, Department of Computer Science, University 
 ...
 */
public enum ParamType { STRING, SET, LONG, UNDEFINED }
...
private Vector<Pair<ParamType,Object>> scanList(String list, Int curInt, char terminator, char separator) 
	...
	while ((paramEnd = scanFor(list,++cur,delim))!=-1) {
		...
		if (obj!=null) ret.add(new Pair<ParamType,Object>(t,obj));
		cur = paramEnd;
...
private String toString(Vector<Pair<ParamType,Object>> v) {
	...
	for (Iterator<Pair<ParamType,Object>> i = v.iterator(); i.hasNext();) {
		...
		switch (p.getKey()) {
			case STRING:
				s.append(Entity.formatID((String)p.getValue()));
				break;
			case SET:
...
/**
 * Scans for the first occurrance of any of the characters in <em>chars</em>
 ...
 * the any target character that is preceded by a backslash.  Thus, scanFor()
 * can be used in conjunction with escape() and unescape() to guarantee that
 * an unknown string can be parsed.  For example, you need to delineate an
 * arbitrary string for future readin, but there may be further text after
 ...
 * @param startAt The offset from the beginning of the <em>in</em> String to start scanning at.
 * @param chars The set of chars to scan for.
 ...
 * s = unescape(s,"()");     (())
 * </pre>
 * @param in The string to remove escapes from.
 * @param escChars The set of escape characters.
